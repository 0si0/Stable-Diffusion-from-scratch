...
# UNet은 매 step마다 timestep을 input으로 받아야 한다.
# 그래서 timestep을 큰 차원의 time embedding으로 바꿔서 Unet에 넣는다.
class TimeEmbedding(nn.Module):

    def __init__(self, n_embed: int):
        super().__init__()
        # 벡터 길이를 4배로 확장 -> 더 많은 정보를 담도록
...
class UNET(nn.Module):
    ...
    def __init__(self):
        ...
        self. decoders = nn.ModuleList(
            ...
            SwitchSequential(UNET_ResidualBlock(2560, 1280), UNET_AttentionBlock(8, 160)),
            SwitchSequential(UNET_ResidualBlock(1920, 1280), UNET_AttentionBlock(8, 80), Upsample(1280)),
...
class Diffusion(nn.Module):
    ...
    def forward(self, latent: torch.Tensor, context: torch.Tensor, time: torch.Tensor):
        ...
        # context = BatchSize, Sequnece_Length, dim
